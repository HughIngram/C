/*
'make install' will overwrite this file and the header with an auto-generated default.
steps required to compile and link this file after modification as per portfolio excersises:
	compile this file to an object file:
>hugh@tinkpad:~/2901/ds/bst$ gcc -g -c bst_char.c
	add the object file to the archive liblinked_bsts.a
>hugh@tinkpad:~/2901/ds/bst$ ar rcs /home/hugh/lib/liblinked_bsts.a bst*.o
>hugh@tinkpad:~/2901/ds/bst$ cp bst_char.h /home/hugh/include/bst_char.h 
*/
...
struct bst_char_implementation
{
    struct node * root;
    int size;
};
...
int  get_min_depth(struct node *p)
{
    int l,r;
    if (p==NULL)
       return 0;
...
int bst_char_min_depth(bst_char *t)
{
    assert(t!=NULL);
    return get_min_depth(t->root);
}

int bst_char_isbalanced(bst_char *t)
{
    assert(t!=NULL);
    return ( bst_char_min_depth(t) == bst_char_depth(t) 
             || bst_char_min_depth(t) == bst_char_depth(t) +1 
             || bst_char_min_depth(t) + 1 == bst_char_depth(t)); 
}

struct node * iterate(struct node *p, char (*fun)(char item))
{
    if (p!=NULL)
    {
       p->left  = iterate(p->left,fun);
       p->right = iterate(p->right,fun);
       p->item  = fun(p->item);
    }
    return p;
}
