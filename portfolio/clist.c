...
//function added as per portfolio excersise
//almost identical to clist_delete
int clist_backspace(clist *c)
{
    assert(c!=NULL);
    clist_goto_prev(c);		//this line has been added
    struct node * p = c->cursor;
    if (clist_cursor_inlist(c))
    {
        c->cursor = c->cursor->next;
    ...
    {
        clist_goto_next(c);	//this line has been added
        return 0;
    }
}
